...
public class Game {
    ...
    private boolean surrender; // if someone decides to surrender, becomes true and stops the loop in
                               // gameFlow()
   ...
   // private OutputStream o; */
    ...
    /**
     * Starts the game loading the game field, settins starting discs and sets the
     * first player to white
     * 
     * @throws IOException
     * 
     */
    ...
    /**
     * Sets and updates the counters for the number of white/black discs on the
     * board
     * 
     */
    ...
    /**
     * Represents the flow of the game, getting user turn inputs, processing them
     * etc.
     * 
     * @throws IOException
     * 
    ...
    public boolean gameFlow() throws IOException {
        ...
        boolean validMove = false; // determines if user's input was 'grammatically' correct and can be further
        ...
        // temporary variables used to check if the previous&current move were both
        // skipped
        boolean one;
        boolean two;
        ...
        do {
            ...
            // if both the first move was made and it was legal, OR if previous turn was
            // skipped, change the current player and reset skippedTUrn variable
            ...
            // if there are available moves - print them, it there are no available moves,
            // print an appropriate message and skip the turn
            if (movesList.size() == 0) {
                moves = false;
                skippedTurn = true;
            ...
            // if both previous player's turn and current player's turn was skipped, end the
            // game
    ...
    /**
     * Updates and prints the counters for the number of white/black discs on the
     * board
     * 
     * @throws IOException
     */
    ...
    public boolean checkTurnInput(String userInput) throws IOException {
        ...
        // if not equal to 2, it is incorrect, else - check if the first is a letter
        // that can be further processed into a horizontal coordinate (a,b,c,d,e,f,g,h)
        ...
        if (inputLength != 2) {
            ...
            if (!letter.equals("a") && !letter.equals("b") && !letter.equals("c") && !letter.equals("d")
                    && !letter.equals("e") && !letter.equals("f") && !letter.equals("g")
            ...
            // tries to get the numeric value from the second part of the string, if
            // impossible - turn input is invalid
            try {
    ...
    public boolean checkMoveLegal(String place) {
        ...
        int num;
        int letterNum;
        boolean[] directions = new boolean[8];// a boolean value that tells if the line was formed in a certain
                                              // direction
        ...
        // if player is trying to put a disc onto an empty tile, check if the move is
        // legal, else move is not legal
        if (field[num][letterNum].equals("0")) {
            // store if player's move causes to form an appropriate line of discs (in every
            // direction) to be flipped and flips them if it does
            ...
            // loops through the array of directions and makes a move legal if discs are
            // being flipped in at least one direction
    ...
    public ArrayList<String> getAvailableMoves() {
        ...
        // loops through each tile on the field (field array), checking if the lines
        // form
        for (int tempV = 0; tempV < field.length; tempV++) {
            for (int tempH = 0; tempH < field[0].length; tempH++) {
                // if the line is unoccupied by discs, placing a disc on it would be a valid
                // move, else does nothing
                if (field[tempV][tempH].equals("0")) {
                    // checks if placing a disc on that tile would make a line form for each
                    // direction
                    ...
                    // loops through the directions and checks if line is formed in any of these
                    // directions thus making a move legal
                    for (int i = 0; i < directions.length; i++) {
                        ...
                        if (directions[i] == true) {
                            ...
                            num = tempV + 1; // increase a vertical coordinate by one (the first line on the field
                                             // presented to user is 1st line, but in the field array it's 0th)
     ...
     * @param horPos      represents the horizontal position of the tile to be
     *                    checked
     * @param direction   represents the direction in which to check if the line
     *                    will be formed
     * @param playerColor represents the symbol of the current player ("W" for
     *                    white, "B" for black)
     * @param action      tells the method whether to check if both the move is
     *                    legal and flip the discs, or just check if the move is
     *                    legal
     * 
    ...
    public boolean checkValidLineFormed(int vertPos, int horPos, String direction, String playerColour, String action) {
        ...
        // makes the method work properly and each time it's called to start from the
        // beginning (sometimes, it beginned from the middle without the loop, i don't
        // know why).
        while (infinity == 0) {
            ...
            tempH += deltaH;
            tempV += deltaV;
            // if the next tile in the line is within the field bounds and is opposite
            // player's colour, start checking the line, else no valid line can be formed in
            // that direction
            if (tempH >= 0 && tempV >= 0 && tempH < field[0].length && tempV < field.length
                    && !field[tempV][tempH].equals(playerColour) && !field[tempV][tempH].equals("0")) {
                // while the tiles are within the bonds, check the tiles
                while (tempV >= 0 && tempV < field.length && tempH >= 0 && tempH < field[0].length) {
                    // if the next tile of the line is empty, no valid lime can be formed in that
                    // direction, else if next tile contains an opponent's disc, a valid line was
                    // formed
                    // and then break out of loop, if neither of these two happened, check the next
                    // tile
                    if (field[tempV][tempH].equals("0")) {
                        lineFormed = false;
                        break;
            ...
            // if line was formed and the command to the method was equal to 'domove', flip
            // discs
            if (lineFormed == true && action.equals("domove")) {
                // go backwards, subtracting delta variables from temporary variables until the
                // coordinates are equal to the line beginning coordinates
    ...
    /**
     * Surrenders the game, displaying the winner and setting the surrender
     * variable's value so that it will break out of gameFlow infinite loop and
     * return to main menu
     * 
     * @throws IOException
     * 
